...
### Costruttore di schedina (homepage)
- **Inserisci le quote** di una giornata: 1X2 e (facoltative) Over/Under 2.5, scegliendo le squadre dai menu a tendina
- **Due mercati**: per ogni partita il modello valuta sia l'1X2 sia l'Over/Under 2.5 e propone l'esito più solido dei due — l'Over/Under è spesso più affidabile, essendo una scelta fra due esiti invece che fra tre
- **Selezione automatica** delle partite più solide, con soglia di confidenza regolabile
- **Probabilità reale** che la schedina esca piena, moltiplicatore e ritorno atteso
- **Distribuzione completa** degli esiti corretti (Poisson binomiale): quanti ne azzecchi realisticamente, e la probabilità di sbagliarne al massimo una
- **Confronto per partita** fra ciò che dice il modello statistico e ciò che dicono le quote

La leva principale non è il modello ma **quali partite includi**: misurato su 12.459 partite, una schedina da 13 passa dallo 0,03% (favorito su tutte le partite) al 3,6% (solo partite con confidenza ≥ 70%).

### Pronostico su singola partita
...
```
PredictA/
├── app.py                                  # Homepage: costruttore di schedina
├── schedina.py                             # Logica schedina: Shin, Over/Under, Poisson binomiale, ritorno atteso
├── pronostico.py                           # Logica di previsione sulla singola partita
├── modello.py                              # Modulo condiviso: Dixon-Coles, decadimento temporale, Shin, RPS, Elo
├── multilega.py                            # Componenti del modello su 5 campionati (test set allargato)
...
│
├── pages/
│   ├── 1_Pronostico.py                     # Pronostico su singola partita (era la vecchia homepage)
...
│   ├── test_multilega.py                   # Test multi-lega, incluso il controllo diretto contro il data leakage
│   └── test_schedina.py                    # Test schedina: Poisson binomiale verificata contro la binomiale
│
...
### Costruttore di schedina (homepage)
1. Inserisci le quote 1X2 della giornata nella tabella, oppure attiva **Precarica un esempio**
2. Regola **confidenza minima** e **numero massimo di partite** nella barra laterale
3. Leggi moltiplicatore, probabilità di schedina piena e ritorno atteso
...
- **Squadre**: 53
- **Colonne**: Date, HomeTeam, AwayTeam, FTHG, FTAG, FTR, Stagione, `OddsAvgH/D/A` (quota di consenso multi-bookmaker in apertura), `OddsAvgCH/CD/CA` (quota di consenso in chiusura, 2.660 partite dal 2019), `OddsOver25`/`OddsUnder25` e `OddsCOver25`/`OddsCUnder25` (Over/Under 2.5 gol, 8.552 e 2.660 partite), B365H/D/A, PSH/D/A, oltre a tiri/corner/cartellini quando disponibili (testati e non usati dal modello di produzione)
